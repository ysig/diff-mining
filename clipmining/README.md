...
a) **Difference**: We compute the difference between `sim(patch, f"{country}") - sim(patch, f"") `.  
b) **Softmax**: We compute the difference between `softmax(\[sim(patch, f"{country}"), sim(patch, f"")\])[0]`.  
c) **Similarity**: We compute the difference between `sim(patch_i, f"{country}")`.
