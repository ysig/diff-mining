...
class SD(object):
  def __init__(self, model_path, categories, device, xformers=True):
    self.device = device
    self.clip_name = 'geolocal/StreetCLIP'
    ...
    self.country_features = CategoryFeatures(self.clip, self.clip_tokenizer, self.device)
...
class D(object):
  def __init__(self, sd, typicallity_path, seed=42, N=100, t_min=0.0, t_max=1.0):
    self.typicallity_path = typicallity_path
    self.sd = sd
    self.seed = seed
    self.N = N
    self.t_min = t_min
  ...
  def get_path(self, path):
    return join(self.typicallity_path, os.path.split(path)[1].replace('.jpg', '.npy').replace('.png', '.npy'))

  def compute(self, country, path):
      img = PIL.Image.open(path)
      seed = os.path.split(path)[1]
      path = join(self.typicallity_path, seed.replace('.jpg', '.npy').replace('.png', '.npy'))
...
class Typicality(object):
  def __init__(self, model_path, dataset_path, typicality_path, t_min=0.0, t_max=1.0, xformers=False):
    self.load_paths(dataset_path)
    self.device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
    if model_path is not None:
      self.sd = SD(model_path, self.categories(), self.device, xformers=xformers)
    else:
      self.sd = None
    self.D = {category: D(self.sd, join(typicality_path, category), t_min=t_min, t_max=t_max) for category in self.categories()}

  def load_paths(self, dataset_path):
    self.parent = {}
  ...
  def get_seeds_(self, c):
    return [path[0] for path in self.country_path[c]]
...
def export_model(args):
  ...
  train_path = 'finetuning/train-4.py'
...
if __name__ == "__main__":
  ...
  mathieu = Typicality(model_path, args.dataset_path, args.typicality_path, t_min=args.t_min, t_max=args.t_max)
