...
NEGATIVE_PROMPT = 'A face portrait'
...
class FTT(Dataset):
    def __init__(self, data_path):
        self.data = []
        for time in os.listdir(join(data_path, 'train')):
            for image in os.listdir(join(data_path, 'train', time)):
                self.data.append((join(data_path, 'train', time, image), time))
    ...
    def __getitem__(self, i):
        ...
        i = np.random.choice(2, p=[0.1, 0.9])

        if i >= 1:
            prompt = prompt + ' of the ' + time + 's.'
...
class Trainer(BaseTrainer):
    ...
    def training_init(self):
        ...
        if self.accelerator.is_main_process:
            self.accelerator.init_trackers("sd-v15-ftt", config=vars(self.args))
    ...
    def sample(self, countries, num_samples=5, steps=50, eta='all', seed=42, ddim=True, guidance_scale=7.5):
        ...
        for country in countries:
            prompt, negative_prompt = [NEGATIVE_PROMPT + ' from the ' + country + 's.']*num_samples, [NEGATIVE_PROMPT]*num_samples
