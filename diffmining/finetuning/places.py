...
class G3r(Dataset):
    def __init__(self, data_path, probabilistic=False):
        ...
        for l in os.listdir(data_path):
            for category in os.listdir(join(data_path, l)):
                for fp in os.listdir(join(data_path, l, category)):
                    if os.path.isdir(join(data_path, l, category, fp)):
                        for fpp in os.listdir(join(data_path, l, category, fp)):
                            assert os.path.isfile(join(data_path, l, category, fp, fpp))
                            self.data[fp + '_' + category].append(join(data_path, l, category, fp, fpp))
                    else:
                        self.data[category].append(join(data_path, l, category, fp))
        self.ids = [(c, v) for c, vs in self.data.items() for v in vs]
    ...
    def process_image(self, img):
        ...
        img = to_tensor(img)
        img = RandomCrop((512, 512))(img)
        img = 2*img - 1.0
        return img

    def __getitem__(self, i):
        c, fp = self.ids[i]
        img = self.process_image(Image.open(fp))
        if random.random() > 0.05:
            prompt = 'Image of ' + c.replace('_', ' ') + '.'
        else:
            prompt = ''
...
def init_dataloader(data_path, probabilistic=False, random_subset=None, shuffle=True, batch_size=8, num_workers=10):
    data = G3r(data_path, probabilistic=probabilistic)
...
class Trainer(BaseTrainer):
    def __init__(self):
        super().__init__()
        self.countries = ['art school', 'army base', 'mosque', 'topiary garden', 'tundra']
        self.init_model()
    ...
    def training_init(self):
        ...
        if not self.args.export_only:
            ...
            self.clip, self.unet = self.accelerator.prepare(self.clip, self.unet)
    ...
    def sample(self, countries, num_samples=5, steps=50, eta='all', seed=42, ddim=True, guidance_scale=7.5):
        ...
        if seed is not None:
            ...
            # dim = 54 * pipeline.vae_scale_factor
            latents_shape = (num_samples, pipeline.unet.in_channels, 64, 64)
            latents = randn_tensor(latents_shape, device=self.accelerator.device, dtype=self.weight_dtype)

        logs = {}
        for country in countries:
            prompt, negative_prompt = [f'An image of {country}.']*num_samples, [NEGATIVE_PROMPT]*num_samples
